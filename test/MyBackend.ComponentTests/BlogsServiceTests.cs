...
internal sealed class BlogsServiceTests : TestsBase
{
    [Test]
    public async Task AddTag_Ok()
    {
        ...
        await BlogsService.AddTagAsync(tag1Value);
        await BlogsService.AddTagAsync(tag2Value);
        await BlogsService.AddTagAsync(tag3Value);
        var tags = await BlogsService.GetTagsAsync();
    ...
    public async Task AddBlog_Ok()
    {
        ...
        var tagId = await BlogsService.AddTagAsync(tagValue);
        
        await BlogsService.AddEntryAsync(new BlogEntryCreateRequest(offset1, "Test blog", "Text", []));
        
        await BlogsService.AddEntryAsync(new BlogEntryCreateRequest(
            new DateTimeOffset(2024, 04, 20, 12, 30, 0, TimeSpan.Zero),
            "Test blog 2",
            "Text",
            []));

        var entryId3 = await BlogsService.AddEntryAsync(new BlogEntryCreateRequest(
            offset3,
            "Test blog 3",
            "Text",
            [tagId]));
        
        var entriesByTagPage = await BlogsService.GetEntriesAsync(tagId: tagId);
        ...
        var entries = await BlogsService.GetEntriesAsync();
